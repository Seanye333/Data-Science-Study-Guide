...
"check": 'assert abs(gate_sigmoid(0) - 0.5) < 1e-12\nassert gate_sigmoid(10) > 0.99\nprint("All checks passed \\u2713")',
        ...
        "code2_title": "GRU Sentiment Classifier",
        "code2": (
            "import torch\n"
            "import torch.nn as nn\n"
            "\n"
        ...
        "code3_title": "LSTM vs GRU Performance Comparison",
        "code3": (
            "import torch\n"
            "import torch.nn as nn\n"
            "import time\n"
        ...
        "practice": {
            "title": "Sequence Direction Predictor",
            "desc": "Build a GRU model with 30-day windows for direction prediction. Compare bidirectional vs unidirectional GRU and report validation accuracy.",
            "starter": (
            "import torch, torch.nn as nn, numpy as np\n"
            "torch.manual_seed(42); np.random.seed(42)\n"
...
"check": 'assert abs(finetune_lr(0.1, 1) - 0.09) < 1e-12\nassert finetune_lr(0.1, 0) == 0.1\nprint("All checks passed \\u2713")',
        ...
        "code2_title": "Gradual Unfreezing with Layer-wise LRs",
        "code2": (
            "import torch\n"
            "import torch.nn as nn\n"
            "\n"
        ...
        "code3_title": "Transfer Learning Diagnostics",
        "code3": (
            "import torch, torch.nn as nn, numpy as np\n"
            "\n"
            "torch.manual_seed(42)\n"
        ...
        "practice": {
            ...
            "desc": "Implement transfer learning on a pretrained encoder: Phase 1 trains only the new head, Phase 2 unfreezes all layers with layer-wise LRs (1e-5 for encoder, 1e-3 for head).",
            "starter": (
...
"check": 'import numpy as np\nassert abs(scaled_dot_attention([1, 0], [1, 0]) - 1/np.sqrt(2)) < 1e-9\nprint("All checks passed \\u2713")',
        ...
        "code2_title": "Transformer Encoder Block",
        "code2": (
        ...
        "code3_title": "Attention Visualization",
        "code3": (
            "import torch, torch.nn as nn, numpy as np\n"
            "\n"
            "torch.manual_seed(42)\n"
        ...
        "practice": {
            ...
            "desc": "Implement scaled dot-product attention and test with batch_size=4, seq_len=8, d_model=16. Verify attention weights sum to 1 along the last dimension.",
            "starter": (
            "import torch, torch.nn as nn\n"
            "\n"
            "def sdp_attention(Q, K, V, mask=None):\n"
...
"check": 'm, v = batchnorm_stats([1, 3])\nassert m == 2.0 and v == 1.0\nprint("All checks passed \\u2713")',
        ...
        "code2_title": "LayerNorm vs BatchNorm",
        "code2": (
            "import torch, torch.nn as nn\n"
            "\n"
            "torch.manual_seed(42)\n"
        ...
        "code3_title": "Dropout Modes & Inference",
        "code3": (
            "import torch, torch.nn as nn, numpy as np\n"
            "\n"
            "torch.manual_seed(42)\n"
        ...
        "practice": {
            "title": "Regularized Classifier",
            "desc": "Build a 3-layer MLP with BatchNorm + Dropout (p=0.3) for binary classification. Compare train vs val accuracy with and without regularization.",
            "starter": (
            "import torch, torch.nn as nn\n"
            "torch.manual_seed(42)\n"
...
"check": 'assert step_lr(1.0, 2) == 0.5 and step_lr(1.0, 0) == 1.0\nprint("All checks passed \\u2713")',
        ...
        "code2_title": "Warmup + Cosine Decay",
        "code2": (
            "import torch, torch.nn as nn, math\n"
            "\n"
            "class WarmupCosine:\n"
        ...
        "code3_title": "ReduceLROnPlateau & LR Finder",
        "code3": (
            "import torch, torch.nn as nn, numpy as np\n"
            "\n"
            "torch.manual_seed(42)\n"
        ...
        "practice": {
            "title": "Warmup + Cosine LR Scheduler",
            "desc": "Implement a warmup+cosine LR scheduler from scratch. Apply it to a regression model and print LR at steps [0, 50, 100, 200, 500].",
            "starter": (
            "import torch, torch.nn as nn, math\n"
            "\n"
...
"check": 'assert should_checkpoint(1.0, 0.5) is True\nassert should_checkpoint(0.5, 1.0) is False\nprint("All checks passed \\u2713")',
        ...
        "code2_title": "Early Stopping with Patience",
        "code2": (
        ...
        "code3_title": "Training Manager (Checkpoint + Early Stop)",
        "code3": (
        ...
        "practice": {
            ...
            "desc": "Write an EarlyStopping class with patience=10 and min_delta=1e-3 that restores best weights on trigger. Verify it stops training and the loaded model has the best val_loss.",
            "starter": (
            "import torch, torch.nn as nn, copy\n"
            "\n"
            "class EarlyStopping:\n"
...
"check": 'import numpy as np\nassert abs(np.linalg.norm(clip_grad([3, 4], 1.0)) - 1.0) < 1e-9\nassert clip_grad([0.1, 0.0], 1.0) == [0.1, 0.0], "small gradients pass through"\nprint("All checks passed \\u2713")',
        ...
        "code2_title": "Mixed Precision Training Pattern",
        "code2": (
        ...
        "code3_title": "Gradient Monitoring & Debugging",
        "code3": (
            "import torch, torch.nn as nn, numpy as np\n"
            "\n"
            "torch.manual_seed(42)\n"
        ...
        "practice": {
            "title": "Gradient Clipping + Monitoring",
            "desc": "Train a 3-layer RNN and clip gradients with max_norm=1.0. Print gradient norm before and after clipping every 10 epochs.",
            "starter": (
            "import torch, torch.nn as nn\n"
            "\n"
...
"check": 'assert export_shapes((3, 32, 32), 8) == (8, 3, 32, 32)\nprint("All checks passed \\u2713")',
        ...
        "code3_title": "Production Deployment Checklist",
        "code3": (
        ...
        "practice": {
            "title": "Export and Verify a Model",
            "desc": "Train a simple classifier, export it via TorchScript (script), save to disk, reload, and verify predictions match with torch.allclose.",
            "starter": (
            "import torch, torch.nn as nn\n"
            "\n"
            "class Net(nn.Module):\n"
